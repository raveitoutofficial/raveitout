...
if getenv("PlayMode") == "Easy" or getenv("PlayMode") == "Special" then return end
...
local function GetNotefieldX()
	return CenterGameplayWidgets() and SCREEN_CENTER_X or THEME:GetMetric("ScreenGameplay","Player"..pn.."OnePlayerOneSideX")
end;
...
local function GetNotefieldWidth(player)
...
local function Border(width, height, bw)
...
local bothWantBars = isTwoPlayers and (ActiveModifiers["P1"].TargetScore and ActiveModifiers["P2"].TargetScore)
local notefield_is_centered = CenterGameplayWidgets()
...
-- (A-, A, A+, etc)
local percentsToDraw = {70,80,90,95,100}
...
for i=1,#percentsToDraw do
	...
	if not (isTwoPlayers and bothWantBars and player == PLAYER_2 and not IsUsingWideScreen()) then
		graph_bg[#graph_bg+1] = Def.BitmapText{
			Font="letters/_ek mukta Bold 24px",
	...
	if use_smaller_graph then
		...
		player_af[#player_af+1] = Def.ActorFrame {
			...
			--[[LoadFont("Common Normal")..{
				...
				UpdateCommand=function(self)
					--self:settext(target_grade.score)
					local targetDP = target_grade.score * GetCurMaxPercentDancePoints()
					self:settext(-percentToYCoordinate(targetDP))
					--self:settext(pss:GetCurrentPossibleDancePoints().."/"..pss:GetPossibleDancePoints())
				end
			};]]
		...
		player_af[#player_af+1] = Def.ActorFrame {
			...
			Def.Quad{
				...
				--[[UpdateCommand=function(self)
					local dp = pss:GetPercentDancePoints()
					self:croptop((graph.h*math.pow(100,dp)/100))
				end]]
				UpdateCommand=function(self)
					local dp = pss:GetPercentDancePoints()
					self:zoomy(-percentToYCoordinate(dp))
			...
			--[[LoadFont("Common Normal")..{
				...
				UpdateCommand=function(self)
					local dp = pss:GetPercentDancePoints()
					self:settext((math.pow(100,dp)/100))
				end
			};]]
		...
		player_af[#player_af+1] = Def.ActorFrame{
			LoadFont("letters/_ek mukta Bold 24px")..{
				...
				InitCommand=function(self)
					self:xy( bar.offset + bar.spacing + (bar.w/2), 8 ):vertalign(top):zoom(.75);
				end,
			},
			...
			LoadFont("letters/_ek mukta Bold 24px")..{
				...
				InitCommand=function(self)
					self:xy( bar.offset + (bar.spacing * 2) + (bar.w/2) + bar.w, 8 ):vertalign(top):zoom(.75);
				end,
			},
			...
			LoadFont("letters/_ek mukta Bold 24px")..{
				...
				InitCommand=function(self)
					self:xy( bar.offset + (bar.spacing * 3) + (bar.w/2) + bar.w * 2, 8 ):vertalign(top):zoom(.75);
				end,
			},
		}
	end
...
if true then
	...
	player_af[#player_af+1] = Def.BitmapText{
		...
		InitCommand=function(self)
			...
			elseif false then self:addx(player==PLAYER_1 and (width/2.75) or -(width/3.5) )
			end
		end,
		UpdateCommand=function(self)
